...
exports.BitbucketDataCenter = void 0;
const n8n_workflow_1 = require("n8n-workflow");
class BitbucketDataCenter {
    constructor() {
        this.description = {
        ...
        this.methods = {
            loadOptions: {
                async getProjects() {
                    ...
                    try {
                        const responseData = await makeApiRequestForOptions.call(this, 'GET', `${baseURL}/rest/api/1.0/projects?limit=100`, undefined, undefined, authentication);
                ...
                async getRepositories() {
                    ...
                    const baseURL = credentials.server;
                    try {
                        const responseData = await makeApiRequestForOptions.call(this, 'GET', `${baseURL}/rest/api/1.0/projects/${projectKey}/repos?limit=100`, undefined, undefined, authentication);
...
exports.BitbucketDataCenter = BitbucketDataCenter;
// Helper function to make API requests for loadOptions
async function makeApiRequestForOptions(method, url, body, qs, authentication) {
    const options = {
        method,
        body,
        qs,
...
async function makeApiRequest(method, url, body, qs, authentication) {
    const options = {
        method,
        body,
        qs,
...
async function getCurrentUsername(baseURL, authentication) {
    try {
        ...
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Failed to get current username: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
// Project operations handler
async function handleProjectOperations(index, operation, baseURL, authentication) {
    if (operation === 'getAll') {
        const returnAll = this.getNodeParameter('returnAll', index);
        const limit = returnAll ? undefined : this.getNodeParameter('limit', index);
    ...
    if (operation === 'get') {
        const projectKey = this.getNodeParameter('projectKey', index);
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "project"`);
}
// Repository operations handler
async function handleRepositoryOperations(index, operation, baseURL, authentication) {
    const projectKey = this.getNodeParameter('projectKey', index);
    if (operation === 'getAll') {
        const returnAll = this.getNodeParameter('returnAll', index);
        const limit = returnAll ? undefined : this.getNodeParameter('limit', index);
    ...
    const repositorySlug = this.getNodeParameter('repositorySlug', index);
    if (operation === 'get') {
    ...
    if (operation === 'create') {
        ...
        const repositorySlugParam = this.getNodeParameter('repositorySlug', index);
        ...
        const isPublic = this.getNodeParameter('public', index);
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "repository"`);
}
// Pull Request operations handler
async function handlePullRequestOperations(index, operation, baseURL, authentication) {
    const projectKey = this.getNodeParameter('projectKey', index);
    const repositorySlug = this.getNodeParameter('repositorySlug', index);
    if (operation === 'getAll') {
        const returnAll = this.getNodeParameter('returnAll', index);
    ...
    const pullRequestId = this.getNodeParameter('pullRequestId', index);
    if (operation === 'get') {
    ...
    if (operation === 'updateComment') {
        const commentId = this.getNodeParameter('commentId', index);
        const body = {
            text: this.getNodeParameter('text', index),
        };
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "pullRequest"`);
}
// Issue operations handler (Note: Bitbucket Data Center may not have native issues, this is for external issue trackers)
async function handleIssueOperations(index, operation, baseURL, authentication) {
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Issue operations are not directly supported in Bitbucket Data Center. Please use Jira integration instead.');
}
// Branch operations handler
async function handleBranchOperations(index, operation, baseURL, authentication) {
    const projectKey = this.getNodeParameter('projectKey', index);
    const repositorySlug = this.getNodeParameter('repositorySlug', index);
    if (operation === 'getAll') {
        const returnAll = this.getNodeParameter('returnAll', index);
    ...
    if (operation === 'delete') {
        const branchName = this.getNodeParameter('name', index);
        const url = `${baseURL}/rest/branch-utils/1.0/projects/${projectKey}/repos/${repositorySlug}/branches`;
        const body = {
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "branch"`);
}
// Commit operations handler
async function handleCommitOperations(index, operation, baseURL, authentication) {
    const projectKey = this.getNodeParameter('projectKey', index);
    const repositorySlug = this.getNodeParameter('repositorySlug', index);
    if (operation === 'getAll') {
        const returnAll = this.getNodeParameter('returnAll', index);
    ...
    if (operation === 'get') {
        const commitId = this.getNodeParameter('commitId', index);
        const url = `${baseURL}/rest/api/1.0/projects/${projectKey}/repos/${repositorySlug}/commits/${commitId}`;
        return await makeApiRequest.call(this, 'GET', url, undefined, undefined, authentication);
    }
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "commit"`);
}
// User operations handler
async function handleUserOperations(index, operation, baseURL, authentication) {
    if (operation === 'get') {
        const username = this.getNodeParameter('username', index);
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "user"`);
}
// Tag operations handler
async function handleTagOperations(index, operation, baseURL, authentication) {
    const projectKey = this.getNodeParameter('projectKey', index);
    const repositorySlug = this.getNodeParameter('repositorySlug', index);
    if (operation === 'getAll') {
        const returnAll = this.getNodeParameter('returnAll', index);
    ...
    if (operation === 'create') {
        ...
        const startPoint = this.getNodeParameter('startPoint', index);
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "tag"`);
}
// File operations handler
async function handleFileOperations(index, operation, baseURL, authentication) {
    var _a;
    const projectKey = this.getNodeParameter('projectKey', index);
    const repositorySlug = this.getNodeParameter('repositorySlug', index);
    if (operation === 'get') {
        const filePath = this.getNodeParameter('filePath', index);
        const at = this.getNodeParameter('at', index, 'main');
    ...
    if (operation === 'getAll') {
        const directoryPath = this.getNodeParameter('directoryPath', index);
        const at = this.getNodeParameter('at', index, 'main');
        const returnAll = this.getNodeParameter('returnAll', index);
        const limit = returnAll ? undefined : this.getNodeParameter('limit', index);
        ...
        const responseData = await makeApiRequest.call(this, 'GET', url, undefined, qs, authentication);
        return ((_a = responseData.children) === null || _a === void 0 ? void 0 : _a.values) || [];
    }
    if (operation === 'create') {
        const filePath = this.getNodeParameter('filePath', index);
        const content = this.getNodeParameter('content', index);
        const commitMessage = this.getNodeParameter('commitMessage', index);
        const branch = this.getNodeParameter('branch', index, 'main');
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "file"`);
}
// Webhook operations handler
async function handleWebhookOperations(index, operation, baseURL, authentication) {
    const projectKey = this.getNodeParameter('projectKey', index);
    const repositorySlug = this.getNodeParameter('repositorySlug', index);
    if (operation === 'getAll') {
        const returnAll = this.getNodeParameter('returnAll', index);
    ...
    if (operation === 'create') {
        const webhookUrl = this.getNodeParameter('url', index);
        const events = this.getNodeParameter('events', index);
        const active = this.getNodeParameter('active', index);
        const body = {
    ...
    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation "${operation}" is not supported for resource "webhook"`);
}
